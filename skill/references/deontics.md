...
- **verb** — Cast, Play, Activate, Attack, Block, Target, Crew, Regenerate,
  Pay-as-cost, …
...
| **MayIf**(a, cost) | a permitted *only if* a toll is paid | — | ward 702.21a, "unless … pays {N}" |
...
by the maximize-without-violating solver (§4). **MayIf** is a conditional
permission whose condition is a payment the actor is never *required* to make
— 508.1d / 509.1c state explicitly that a creature that "can't attack/block
unless a player pays a cost" does **not** oblige that player to pay, even to
...
obeyed subject to every restriction, with the toll caveat above. Goad piles
on — each goading player adds requirements (701.15c); a goaded creature
"attacks each combat if able and attacks a player other than [the goader] if
able" (701.15b), which is **Must**(Attack) ∧ **Cant**(Attack the goader) in
composition.
...
creatures", 702.111b) and banding are not per-creature checks — they
constrain the *whole declaration set*. 509.1c evaluates the entire blocker
set against the menace restriction (its own example walks a two-blocker
requirement against menace). The legality predicate's argument is the
declaration set, not one creature at a time.
...
| **Goad** (701.15b–c) | `Must(Attack(This)) ∧ Cant(Attack-target(This, goader))`; multiple goaders ⇒ additional requirements (701.15c) |
...
legal(a):                          # a = action instance, choices may be open
    ...
    return ∃ c ∈ completions:
        ...
        ∧ (¬∃ row ∈ restrictions(verb(a)): row.forbids(c))  # Cant subtraction, 101.2
...
declare_combat(player, side):                       # attackers 508 / blockers 509
    ...
    legal_sets = { S ∈ sets : ∀ member: legal(...)        # per-member Cant
                              ∧ set_predicates_ok(S) }     # menace/banding, set-level
