...
# The `rule N` net uses a `(?<![\w-])` lookbehind rather than `\b`: a hyphen is
# a word boundary, so `\brule 2` fired on CLI flags like `--per-rule 2`. Do not
# "simplify" it back.
...
| [($t | [match("CR ?[0-9]{1,3}(\\.[0-9]+[a-z]*)?"; "g")]),
   ($t | [match("(?<![\\w-])rule [0-9]{1,3}(\\.[0-9]+[a-z]*)?"; "g")]),
