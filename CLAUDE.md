...
- `skill/scripts/` — fish lookup tools (`rule`, `rule-search`, `define`, `keyword`, `card`, `rulings`, `mtr`, `corpus`, `check-citations`, `setup-data`); all source `lib.fish`, which resolves data via `$MTG_RULES_DATA` → `~/.claude/plugins/data/mtg-rules/data` → repo-relative.
...
2. **Dotted citation on every claim.** `skill/scripts/check-citations` machine-verifies that every `NNN.Nx` cited anywhere in `references/` and the keywords JSON exists in the current CR — the staleness tripwire for quarterly CR updates.
...
- Verification before completion: `fish tests/test_skill_scripts.fish` (all green) and `skill/scripts/check-citations` (exit 0) before any commit that touches `skill/`.
...
`scripts/fetch_data.fish` → `scripts/build_derived.fish` → `skill/scripts/check-citations` (fix flagged docs) → re-derive the probes in `references/rulings-check.md` → re-test `references/underdetermined.md` entries (a new rule may settle one) → re-examine `references/generalizations.md` empty cells (a newly printed instance is a finding).
