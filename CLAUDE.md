...
This repo is three things at once: a **Claude Code plugin marketplace** (`.claude-plugin/marketplace.json`, plugin `mtg-rules` sourced from `./skill`), the **development home of the mtg-rules skill** (`skill/`), and the **data workspace** that grounds it (`data/`, gitignored, 2.3 GB fetched). The skill's purpose: judge-grade MTG rules answers and implementation-facing taxonomies for an MTG engine + card-grammar project.

## Layout

- `skill/SKILL.md` — triggers, rigid workflow, routing to docs/scripts.
- `skill/references/*.md` — synthesized reference docs (see SKILL.md's table for the roster).
- `skill/scripts/` — fish lookup tools (`rule`, `rule-search`, `define`, `keyword`, `card`, `rulings`, `mtr`, `corpus`, `cite`, `setup-data`); all source `lib.fish`, which resolves data via `$MTG_RULES_DATA` → `~/.claude/plugins/data/mtg-rules/data` → repo-relative.
- `skill/keywords-classified.json` — all keywords classified intrinsic / composite / composite-given(P) / marker.
...
2. **Dotted citation on every claim.** `skill/scripts/cite check` machine-verifies that every `NNN.Nx` cited anywhere in `references/` and the keywords JSON exists in the current CR *and* that its text still matches the lockfile (`skill/cr-citations.lock` pins normalized-text checksums; staleness classes CHANGED/GONE/UNLOCKED) — the staleness tripwire for quarterly CR updates. Accept intended rewording with `cite bless`.
...
- Verification before completion: `fish tests/test_skill_scripts.fish` (all green) and `cd skill && scripts/cite check` (exit 0) before any commit that touches `skill/`.
...
`scripts/fetch_data.fish` → `scripts/build_derived.fish` → `skill/scripts/cite check` (for each CHANGED hit: `cite diff <rule>`, fix the doc or accept the rewording; GONE/UNLOCKED: fix the cite; then `cite bless` to re-pin the lockfile) → re-derive the probes in `references/rulings-check.md` → re-test `references/underdetermined.md` entries (a new rule may settle one) → re-examine `references/generalizations.md` empty cells (a newly printed instance is a finding).
